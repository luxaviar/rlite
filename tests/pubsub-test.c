...
//	int64_t channellen = strlen(CHANNEL);
...
//	int64_t channelslen[2] = {strlen(CHANNEL), strlen(CHANNEL2)};
//
//	struct buf buffer;
...
//	}
//	rl_discard(db);
//	uint64_t before = rl_mstime();
//	pthread_create(&thread_w, NULL, publish, &buffer_w);
//	rl_refresh(db);
//	poll(db, &buffer_r);
//	rl_close(db);
//
//	uint64_t elapsed = rl_mstime() - before;
//	ASSERT(elapsed >= 1000);
...
//
//	const char *pattern = "*";
//	int64_t patternlen = strlen(pattern);
//
//	struct buf buffer;
//	init_buffer(&buffer, NULL, 0, NULL, 0);
//
...
//	int64_t channel2len = strlen(channel2);
//
//	int64_t channelc;
...
//	int64_t channellen = strlen((char *)channel);
//	unsigned char *channel2 = (unsigned char *)CHANNEL2;
//	int64_t channel2len = strlen((char *)channel2);
//	unsigned char *channels[] = {channel, channel2};
//	int64_t channelslen[] = {channellen, channel2len};
//	int64_t count[2] = {-1, -1};
//
//	rlite *db = NULL;
//	RL_CALL_VERBOSE(setup_db, RL_OK, &db, 1, 1);
...
//	int64_t patternlen = strlen((char *)pattern), count;
//
//	rlite *db = NULL;
//	RL_CALL_VERBOSE(setup_db, RL_OK, &db, 1, 1);
//
